...
#         """
#         url = reverse("credit:creditprovider-list") + "?provider_ids=hogwarts"
...
#         url = reverse("credit:creditprovider-list") + "?provider_ids=hogwarts,dummy_id"
#         response = self.client.get(url)
...
#         url = reverse("credit:creditprovider-list")
#         response = self.client.get(url)
#         self.assertEquals(len(json.loads(response.content)), 2)
...
@ddt.ddt
@unittest.skipUnless(settings.ROOT_URLCONF == 'lms.urls', 'Test only valid in lms')
class CreditProviderViewSetTests(ReadOnlyMixin, AuthMixin, UserMixin, TestCase):
    ...
    def test_list_filtering(self, provider_ids):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self._getAssertEqualityFunc(response.data['count'], len(provider_ids))
...
class CreditProviderCallbackViewTests(TestCase):
    def setUp(self):
        self.provider = CreditProviderFactory()
        self.path = reverse('credit:provider_callback', args=[self.provider.provider_id])

    def test_no_auth_required(self):
        """ Verify the endpoint does not require authentication. """
        self.client.logout()
        response = self.client.post(self.path, {})
        self.assertEqual(response.status_code, 200)

    def test_post(self):
        """ Verify the endpoint can receive responses from the external credit provider. """
        self.fail()
    ...
    def test_post_invalid_signature(self):
        """ Verify the endpoint fails if the provided signature is invalid. """
        self.fail()
